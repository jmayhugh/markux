...
// Static trusted SVG — safe for innerHTML (no user input)
 ...
 */
export function createPinMarker(number, x, y, onClick, authorName) {
  const pin = document.createElement("div");
  pin.className = "markux-pin";
  pin.style.left = `${x}px`;
  pin.style.top = `${y}px`;
  pin.innerHTML = PIN_SVG; // eslint-disable-line no-unsanitized/property — static constant, not user input
